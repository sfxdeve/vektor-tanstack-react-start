...
export function getAppUrl(env: MailEnv): string {
  const raw = (env.APP_URL || "").trim();
  if (raw) return raw.replace(/\/$/, "");
  return "http://localhost:3000";
}

export function getSender(env: MailEnv): string {
...
export function getResendApiKey(env: MailEnv): string {
  return (env.RESEND_API_KEY || "").trim();
}

export function isDevMailbox(env: MailEnv): boolean {
  return env.DEV_MAILBOX === "1";
}
...
export async function sendViaResend(
  ...
  if (isDevMailbox(env)) {
    ...
    captureEmail({
      ...
      from,
      subject,
      html: opts?.type === "reset-password" ? "[reset link stored in raw test data]" : html,
      ...
      resendId: id,
      raw: opts?.raw ?? { to: toEmail, from, subject, html, appUrl, threshold: opts?.threshold },
    });
    return id;
  }
...
export async function sendEftDecisionEmail(
  ...
  try {
    await sendViaResend(env, args.to, subject, html, { type: args.type });
  } catch (e) {
