...
function AdminEftPage() {
  ...
  if (paymentsQueryResult.isError) {
    return (
      <div className="py-16 text-center" data-testid="admin-eft-error">
        <p className="text-sm text-red-300">Could not load EFT payments.</p>
        <button
          type="button"
          data-testid="admin-eft-retry"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-bold tracking-[0.2em] text-zinc-400 uppercase">EFT Payments</p>
          <h1
      ...
      <div className="overflow-hidden rounded-sm border border-zinc-800 bg-zinc-950">
        <div className="overflow-x-auto">
          <Table className="text-sm" data-testid="admin-eft-table">
            <TableHeader className="bg-zinc-900">
              <TableRow className="border-b border-zinc-800 hover:bg-transparent">
                <TableHead className="px-4 py-3 text-[11px] uppercase tracking-[0.15em] text-zinc-400">
                  Reference
                </TableHead>
                <TableHead className="px-4 py-3 text-[11px] uppercase tracking-[0.15em] text-zinc-400">
                  User
                ...
                <TableHead className="px-4 py-3 text-right text-[11px] uppercase tracking-[0.15em] text-zinc-400">
                  Amount
                </TableHead>
                <TableHead className="px-4 py-3 text-[11px] uppercase tracking-[0.15em] text-zinc-400">
                  Status
            ...
            <TableBody>
              {paymentsQueryResult.isFetching && (
                <TableRow className="border-b border-zinc-800 hover:bg-transparent">
                  <TableCell colSpan={7} className="px-4 py-10 text-center text-sm text-zinc-400">
                    Loading…
                  </TableCell>
                </TableRow>
              )}
              {!paymentsQueryResult.isFetching && payments.length === 0 && (
                <TableRow className="border-b border-zinc-800 last:border-0 hover:bg-transparent">
                  <TableCell
                    colSpan={7}
              ...
              {!paymentsQueryResult.isFetching &&
                payments.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell className="px-4 py-3 font-mono text-xs font-semibold text-white">
                        {p.reference}
                      </TableCell>
                      ...
                      <TableCell className="px-4 py-3">
                        <span
                          className={`inline-flex items-center gap-1 rounded-sm border px-2 py-0.5 text-[11px] font-bold ${meta.className}`}
                        >
                          {meta.label}
                        </span>
      ...
      >
        <DialogContent
          ...
          className="flex max-h-[90vh] max-w-3xl flex-col gap-0 overflow-hidden rounded-sm border border-zinc-800 bg-zinc-900 p-0 text-white"
        >
          <DialogHeader className="flex-row items-center justify-between border-b border-zinc-800 px-6 py-4">
            <div>
          ...
          {proofPayment?.status === "pending_review" && (
            <DialogFooter className="m-0 flex-row border-zinc-800 bg-zinc-900 px-6 py-4">
              <button
                type="button"
      ...
      >
        <DialogContent
          ...
          className="rounded-sm border-zinc-800 bg-zinc-900 p-6 text-white sm:max-w-md"
        >
          <DialogHeader>
      ...
      >
        <AlertDialogContent
          data-testid="admin-eft-confirm-dialog"
