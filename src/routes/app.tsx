...
function verdictOf(fitScore: number): "GO" | "CAUTION" | "NO-GO" {
  if (fitScore >= 75) return "GO";
  if (fitScore >= 50) return "CAUTION";
  return "NO-GO";
}
...
function AppPage() {
  ...
  // Session resolved but the company list is still loading.
  ...
  // No company yet — keep dashboard-title so the landing redirect test stays green.
                  ...
                  <table className="w-full text-sm">
                      ...
                      <tr>
                        {[
                          ...
                          <th
                            key={heading}
                            scope="col"
                            className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-[0.1em] text-zinc-700"
                          >
                            {heading}
                          </th>
                        ))}
                    ...
                    <tbody className="divide-y divide-zinc-200">
                      {tenders.slice(0, 5).map((tender) => {
                        const verdict = verdictOf(tender.fit_score);
                        return (
                          <tr
                            key={tender.id}
                          ...
                          >
                            <td className="px-6 py-4 font-medium text-zinc-900">{tender.title}</td>
