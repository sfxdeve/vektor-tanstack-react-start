  ...
  server: {
    handlers: {
      ...
      DELETE: async ({ request, params }) => {
        ...
        // Delete stored objects first — the DB cascade removes the rows.
        const docs = await db
        ...
        const tenderKeys = await db
          .select({ pdfStorageKey: tenders.pdfStorageKey })
          .from(tenders)
        ...
        const keys = [
          ...
          ...tenderKeys.map((t) => t.pdfStorageKey),
        ...
        await Promise.all(
          keys.map(async (key) => {
            try {
              ...
              console.warn("R2 delete failed during company cascade", key, e);
            }
          }),
        );
