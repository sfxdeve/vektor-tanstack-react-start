    ...
    handlers: {
      GET: async ({ request, params }) => {
        ...
        const rows = (await db
          .select()
          .from(companies)
          .where(eq(companies.id, companyId) as unknown as never)
          .catch(async () =>
            (await db.select().from(companies)).filter((c) => c.id === companyId),
          )) as (typeof companies.$inferSelect)[];
        const target =
          rows[0] ??
          ((await db.select().from(companies)).find((c) => c.id === companyId) as
        ...
        const docs = (await db
          ...
          .where(eq(complianceDocuments.companyId, companyId) as unknown as never)
          .catch(async () =>
            (await db.select().from(complianceDocuments)).filter(
        ...
        const companyTenders = (await db
          .select()
          .from(tenders)
          .where(eq(tenders.companyId, companyId) as unknown as never)
          .catch(async () =>
            (await db.select().from(tenders)).filter(
        ...
        const credits = (await db
          .select()
          .from(companyCredits)
          .where(eq(companyCredits.companyId, companyId) as unknown as never)
          .catch(async () =>
            (await db.select().from(companyCredits)).filter(
              (c) => (c as unknown as { companyId: string }).companyId === companyId,
            ),
          )) as (typeof companyCredits.$inferSelect)[];
        const reminders = (await db
          .select()
          .from(sentReminders)
          .where(eq(sentReminders.companyId, companyId) as unknown as never)
          .catch(async () =>
            (await db.select().from(sentReminders)).filter(
        ...
        const eft = (await db
          .select()
          .from(eftPayments)
          .where(eq(eftPayments.companyId, companyId) as unknown as never)
          .catch(async () =>
      ...
      DELETE: async ({ request, params }) => {
        ...
        try {
          await db
            ...
            .where(inArray(complianceDocuments.companyId, [companyId]) as unknown as never);
        } catch {
          for (const d of docs) {
            try {
              await (
        ...
        try {
          await db
            .delete(tenders)
            .where(inArray(tenders.companyId, [companyId]) as unknown as never);
        } catch {
          for (const t of tRows) {
            try {
        ...
        try {
          ...
          for (const r of (await db.select().from(sentReminders)).filter(
            (x) => (x as unknown as { companyId: string }).companyId === companyId,
          )) {
            try {
        ...
        try {
          await db
            ...
            .where(eq(referralRewards.referrerCompanyId, companyId) as unknown as never);
        } catch {}
