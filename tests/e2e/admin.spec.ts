...
test.describe("Admin Console (Issue 09)", () => {
  ...
  test("admin can sign out from the console", async ({ page }) => {
    const email = uniqueEmail("admin-signout");
    const password = "correct-horse-battery-staple-123";
    await signupViaUI(page, "Admin Signout", email, password);
    promoteLocalUserToAdmin(email);
    await clearSession(page);
    await loginViaUI(page, email, password);
  ...
  test("admin help keeps admin navigation and returns to the console", async ({ page }) => {
    const email = uniqueEmail("admin-help");
    const password = "correct-horse-battery-staple-123";
    await signupViaUI(page, "Admin Help", email, password);
    promoteLocalUserToAdmin(email);
    await clearSession(page);
    await loginViaUI(page, email, password);
  ...
  test("admin can impersonate a user and exit through the visible banner", async ({ page }) => {
    const password = "correct-horse-battery-staple-123";
    const adminEmail = uniqueEmail("admin-impersonation");
    const userEmail = uniqueEmail("impersonation-target");

    await signupViaUI(page, "Impersonation Admin", adminEmail, password);
    promoteLocalUserToAdmin(adminEmail);
    await clearSession(page);
    await signupViaUI(page, "Impersonation Target", userEmail, password);
    await clearSession(page);
    ...
    const targetRow = page.locator('[data-testid^="admin-user-row-"]', { hasText: userEmail });
    await expect(targetRow).toBeVisible({ timeout: 15000 });
    await targetRow.locator('[data-testid^="admin-user-impersonate-"]').click();
