...
test.describe("EFT Billing", () => {
  ...
  test("payment deep-links show EFT status pages; unknown slugs 404", async ({ page }) => {
    const email = uniqueEmail("payment-link");
    const password = "correct-horse-battery-staple-123";
    await page.goto("/signup");
    await page.waitForLoadState("networkidle");
    await expect(page.getByTestId("signup-form")).toBeVisible();
    await page.getByTestId("input-name").fill("Payment Link User");
    await page.getByTestId("input-email").fill(email);
    await page.getByTestId("input-password").fill(password);
    await page.getByTestId("submit-signup").click();
    await page.waitForURL(/\/app|\/setup/, { timeout: 15000 });
    // Let the post-signup redirect settle before the first goto (webkit
    // interrupts overlapping navigations otherwise — same mitigation as
    // ensureCompanySetup).
    await expect(page.getByTestId("user-email")).toContainText(email, { timeout: 10000 });
    await page.waitForLoadState("networkidle");

    // Success deep-link explains the EFT verification flow.
    await page.goto("/payment/success");
    await page.waitForLoadState("networkidle");
    await expect(page.getByTestId("payment-status-title")).toContainText("Payment Status");
    await expect(page.getByTestId("payment-success-copy")).toBeVisible();

    // Cancel deep-link offers a way back to Billing.
    await page.goto("/payment/cancel");
    await page.waitForLoadState("networkidle");
    await expect(page.getByTestId("payment-cancelled-copy")).toBeVisible();
    await page.getByTestId("payment-back-to-billing-btn").click();
    await expect(page).toHaveURL(/\/billing/);
    await page.waitForLoadState("networkidle");

    // Any other slug is a plain 404 (HTTP-level assert — no SPA navigation,
    // which webkit races when the previous client-side redirect is in flight).
    const notFoundRes = await page.request.get("/payment/bogus");
  ...
  test("bank details endpoint returns FNB fields and package catalog is correct via API", async ({
    ...
    const email = uniqueEmail("bank-details");
    const password = "correct-horse-battery-staple-123";
    await page.goto("/signup");
    await page.getByTestId("input-name").fill("Bank Details User");
    await page.getByTestId("input-email").fill(email);
    await page.getByTestId("input-password").fill(password);
    await page.getByTestId("submit-signup").click();
    await page.waitForURL(/\/app|\/setup/, { timeout: 15000 });
