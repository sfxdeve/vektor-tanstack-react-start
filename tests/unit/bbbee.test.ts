...
import { calculateBbbeePoints, verdictFromScore } from "@/lib/bbbee";
...
describe("bbbee — verdict mapping", () => {
  it("GO at 75+", () => {
    expect(verdictFromScore(75)).toBe("GO");
    expect(verdictFromScore(100)).toBe("GO");
    expect(verdictFromScore(99)).toBe("GO");
  });
  it("CAUTION 50-74", () => {
  ...
  it("NO-GO below 50", () => {
    expect(verdictFromScore(49)).toBe("NO-GO");
    expect(verdictFromScore(0)).toBe("NO-GO");
    expect(verdictFromScore(30)).toBe("NO-GO");
  });
  it("UNKNOWN for null/undefined", () => {
