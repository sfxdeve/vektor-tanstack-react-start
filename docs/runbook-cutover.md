...
nubx wrangler secret put AI_GATEWAY_ID         # optional; configure with account ID below
...
VARS="--var APP_URL=$PROD_URL --var BETTER_AUTH_URL=$PROD_URL --var CLOUDFLARE_ACCOUNT_ID=$CLOUDFLARE_ACCOUNT_ID"
```

`AI_GATEWAY_ID` and `CLOUDFLARE_ACCOUNT_ID` must be configured together; omit
both to call OpenAI directly. `DEV_MAILBOX` is local-only and must exist only in
`.dev.vars`; the endpoint also rejects non-loopback requests. Never set it on a
preview or production upload. Other `DEV_*` flags must never be set in production.

Before cutover, exercise **Forgot password** with a throwaway account and verify
that Resend delivers the reset link, the link changes the password, and old
sessions are revoked. Reset URLs and tokens must never appear in Worker logs.
...
The deterministic smoke does not verify the provider path. Upload a second
preview **without** `DEV_AI_STUB`, then analyze one throwaway tender with the
real `OPENAI_API_KEY` and AI Gateway configuration. Confirm the structured
result renders and the original PDF downloads before production cutover:

```bash
nubx wrangler versions upload $VARS
# open the newly printed preview URL and run one real tender analysis
```
...
- **Worker entry** is `worker.ts`: one default Worker handler object composes
  TanStack Start's fetch handler with the `scheduled` Compliance Guardian
  handler. No build-time patching is involved.
...
- `/payment/success` and `/payment/cancel` are protected EFT status deep-links.
  They are informational application pages, not Stripe callbacks; EFT request
  state still progresses awaiting_proof → pending_review → confirmed/rejected.
...
- R2 replacement/deletion uses uniquely staged objects and DB compare-and-swap where applicable.
  Cleanup after a successful DB commit remains best-effort because D1 and R2 do not share a
  transaction; provider delete failures can leave an orphan that requires operational cleanup.
- `/api/dev/mailbox` requires `DEV_MAILBOX=1` and a loopback hostname. No HTTP endpoint can
  promote a user role; the first admin is created out-of-band through D1.
